...
class LogisticModel(BaseModel):
  ...
  def create_model(self, model_input, num_classes=10, l2_penalty=1e-8, **unused_params):
    ...
    net = slim.repeat_op(2, model_input, slim.conv2d, 32, [2,2], scope='conv1')
    net = slim.max_pool(net, [2,2], scope='pool1')
    net = slim.repeat_op(2, net, slim.conv2d, 64, [2,2], scope='conv2')
...
"""class myModel(BaseModel):
  ...
  def create_model(self, model_input, keep_prob, **unused_params):
    ...
    output=tf.nn.softmax(tf.matmul(h_fc1_drop, W_fc2) +v b_fc2)
...
#   print model_input,"1111111111111"